...
export function CustomerOrders({ customerId }: CustomerOrdersProps) {
  ...
  const summary = useMemo(
    () => customerService.getOrderSummary(customerId),
    [customerId],
  );
      ...
      {orders.length === 0 ?
        <div className='rounded-xl border border-border/70 bg-card'>
          <EmptyState
            ...
            description='This customer does not have any sales orders yet.'
          />
        </div>
                ...
                {orders.map((order: CustomerOrder) => (
                  <TableRow key={order.id} className='group'>
                    ...
                    <TableCell>
                      <Link
                        href={`/admin/operations/sales-orders/${order.id}`}
                        className='flex size-8 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted hover:text-foreground sm:opacity-0 sm:group-hover:opacity-100'
                      >
                        <ArrowUpRight className='size-4' />

                        <span className='sr-only'>View sales order</span>
                      </Link>
                    </TableCell>
                  </TableRow>
                ))}
